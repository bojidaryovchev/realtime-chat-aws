...
export function createEcsServices(
  ...
  const realtimeLogGroup = new aws.cloudwatch.LogGroup(`${baseName}-realtime-logs`, {
  ...
  // API Task Definition
  const apiTaskDefinition = new aws.ecs.TaskDefinition(`${baseName}-api-task`, {
    family: `${baseName}-api`,
    networkMode: "awsvpc",
  ...
  const realtimeTaskDefinition = new aws.ecs.TaskDefinition(`${baseName}-realtime-task`, {
  ...
  const realtimeAutoScaling = new aws.appautoscaling.Target(`${baseName}-realtime-autoscaling`, {
  ...
  if (config.realtimeScaleOnConnections) {
    ...
    const eventLoopLagScaleOutPolicy = new aws.appautoscaling.Policy(`${baseName}-realtime-eventloop-scaling`, {
