...
flowchart TB
    ...
    subgraph ALB_Layer["ALB Security Group"]
        ALB[⚖️ ALB<br/>Ports: 80, 443<br/>From: 0.0.0.0/0]
    end

    subgraph ECS_Layer["ECS Security Groups"]
        API[🔷 API SG<br/>Port: 3001<br/>From: ALB only]
        RT[🔷 Realtime SG<br/>Port: 3002<br/>From: ALB only]
        Workers[⚙️ Workers SG<br/>Inbound: NONE]
    end

    subgraph Data_Layer["Data Security Groups"]
        RDS[(🐘 RDS SG<br/>Port: 5432<br/>From: ECS only)]
        Redis[(⚡ Redis SG<br/>Port: 6379<br/>From: ECS only)]
    end
...
| Source → Dest | ALB | API | Realtime | Workers | RDS | Redis |
|--------------|-----|-----|----------|---------|-----|-------|
| **Internet** | ✅ 80,443 | ❌ | ❌ | ❌ | ❌ | ❌ |
| **ALB** | - | ✅ 3001 | ✅ 3002 | ❌ | ❌ | ❌ |
| **API** | ❌ | - | ❌ | ❌ | ✅ 5432 | ✅ 6379 |
