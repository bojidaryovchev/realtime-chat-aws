...
flowchart TB
    subgraph Cluster["🔷 ECS Cluster"]
        subgraph Workers["⚙️ Workers Service"]
            Task1["📦 Task (Fargate)<br/>🟢 Workers Container<br/>Port 3003"]
            Task2["📦 Task (Fargate)<br/>🟢 Workers Container<br/>Port 3003"]
            TaskN["📦 ... more tasks"]
        end
    end
    ...
    subgraph DLQ["☠️ Dead Letter Queues"]
        PushDLQ["🔴 Push DLQ"]
        OfflineDLQ["🔴 Offline DLQ"]
    end

    Task1 & Task2 & TaskN -->|Poll messages| Push & Offline
    Push -->|Failed 3x| PushDLQ
    Offline -->|Failed 3x| OfflineDLQ
```
...
sequenceDiagram
    ...
    participant Worker as Workers Task
    participant DLQ as Dead Letter Queue

    API->>SQS: SendMessage
    loop Poll every 1s
        Worker->>SQS: ReceiveMessage (batch 10)
        SQS-->>Worker: Messages
        Worker->>Worker: Process messages
        alt Success
